...
## Why this matters (On-device + privacy)

Transformer quantization is a practical enabler for **on-device inference** where memory, latency, and energy are constrained.
Smaller models can make it feasible to keep inference local, reducing dependency on server-side compute and improving privacy posture.

**Edge-friendly methods (in this repo):**
- **Mixed-precision PTQ** (keep FFN input/output/residual sum in 16-bit) often recovers accuracy with minimal overhead.
- **PEG-PTQ** (per-embedding-group activation quantization) reduces the impact of structured activation outliers.

### On-device pipeline (conceptual)

```mermaid
flowchart LR
  A[User Input] --> B[Tokenizer]
  B --> C[Quantized Transformer (INT8/MP/PEG)]
  C --> D[Prediction]
  C --> E[Profiling: latency/VRAM/size]
  D --> F[On-device Output]
```

---

## One-click reproduction + profiling + paper

Run:

- macOS/Linux: `./run_oneclick.sh`
- Windows: `run_oneclick.bat`

Outputs:
- `runs/` experiment artifacts
- `runs/results/deploy_profile.csv`
- `runs/results/accuracy_latency_size.png`
- `paper/paper.tex` + `paper/references.bib` (compile to PDF)
