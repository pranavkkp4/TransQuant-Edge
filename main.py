...
def _run_task(config, task: GLUE_Task, task_data, model_data):
    ...
    if (per_token or per_embd or per_groups) and model_enum in (
        ...
        for layer_idx in range(12):
            ...
            if hasattr(A, 'context_act_quantizer'):
                set_act_quant_axis_and_groups(
    ...
    if quant_dict and model_enum in (HF_Models.bert_base_uncased, HF_Models.bert_large_uncased):
        ...
        for layer_idx in range(12):
            ...
            # Self-attention
            A = L.attention.self
            if hasattr(A, 'attn_scores_act_quantizer'):
                ...
                logger.debug(f'Layer {layer_idx}: attn_scores_act_quantizer missing; skipping.')

            if hasattr(A, 'attn_probs_act_quantizer'):
                ...
                logger.debug(f'Layer {layer_idx}: attn_probs_act_quantizer missing; skipping.')

            if hasattr(A, 'context_act_quantizer'):
                hijack_act_quant(quant_dict, f'c{layer_idx}', A.context_act_quantizer)
                hijack_act_quant(quant_dict, 'c', A.context_act_quantizer)
            else:
                logger.debug(f'Layer {layer_idx}: context_act_quantizer missing; skipping.')
            ...
            if config.quant.per_groups_permute_shared_h:
                for layer_idx, layer in enumerate(model.bert.encoder.layer):
                    ...
                    for k, v in range_estimators.items():
                        print(k)
                        if 'dense' in k and v.ranges is not None:
                            source_ranges = v.ranges.clone()
                            break

                    if source_ranges is None:
                        logger.warning(
                            f'Layer {layer_idx}: no dense ranges found for sharing; skipping.'
                        )
                        continue
    ...
    if config.training.do_train:
        ...
        # Auto-resume from requested or latest checkpoint if present
        resume_ckpt = config.progress.resume_checkpoint
    ...
    if config.training.do_eval:
        ...
        # save final score to file (always write if an output dir is provided)
        if config.base.output_dir is not None:
            os.makedirs(config.base.output_dir, exist_ok=True)
            with open(os.path.join(config.base.output_dir, 'final_score.txt'), 'w') as f:
                f.write(f'{final_score}\n')

    # For eval-only runs, optionally persist the (quantized) model for downstream profiling
    if config.progress.save_model and not config.training.do_train:
...
def _eval_task(config, task, trainer, eval_dataset, datasets):
    ...
    for subtask, eval_dataset in zip(subtask_names, eval_datasets):
        ...
        # save eval results to files (always, so downstream tools can parse)
        if config.base.output_dir is not None:
            os.makedirs(config.base.output_dir, exist_ok=True)
            subtask_eval_fpath = os.path.join(config.base.output_dir, f'eval_results_{subtask}.txt')
            subtask_eval_json = os.path.join(config.base.output_dir, f'eval_results_{subtask}.json')
            with open(subtask_eval_fpath, 'w') as f:
                for key, value in eval_result.items():
                    f.write(f'{key} = {value}\n')
            with open(subtask_eval_json, 'w', encoding='utf-8') as jf:
                json.dump({k: float(v) for k, v in eval_result.items()}, jf, indent=2)
    ...
    for task in task_flag.iter():
        ...
        if config.model.model_path is None:  # use pre-trained backbone for training/validation
            ...
            if do_train:
                ...
                task_out_dirpath = None
                if len(non_empty_task_out_dirpaths):
                    ...
                    # Fallback: allow direct model_path to already contain checkpoint files
                    direct_path = Path(config.model.model_path)
                    if _is_non_empty_dir(direct_path):
                        task_out_dirpath = str(direct_path)
                        task_config = deepcopy(config)
                        if config.base.output_dir is None:
                            task_config.base.output_dir = task_out_dirpath
                        task_config.model.model_path = task_out_dirpath
