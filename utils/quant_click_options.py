...
def make_qparams(config):
    ...
    act_range_options = dict(config.act_quant.options)
    act_range_method = RangeEstimators[config.act_quant.quant_method]

    # If percentile clipping is requested, use current_minmax (supports percentile)
    if config.quant.percentile is not None:
        act_range_method = RangeEstimators.current_minmax
        act_range_options['percentile'] = config.quant.percentile
        # pass percentile to weight range estimator if supported (CurrentMinMax)
        weight_range_options['percentile'] = config.quant.percentile
    ...
    params = {
        ...
        'act_range_method': act_range_method,
